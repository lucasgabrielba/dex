...
export function PropertyDevTableRow({ row, selected, onSelectRow, onDeleteRow, detailsHref }: Props) {
      ...
      <TableRow hover selected={selected}>
        ...
        <TableCell>
          <Label
            variant="soft"
            color={
              (row.status === 'Pronto' && 'success') ||
              (row.status === 'Em constução' && 'warning') ||
              (row.status === 'Obras pausadas' && 'error') ||
              (row.status === 'Pré lançamento' && 'info') ||
              'default'
            }
          >
            {row.status}
